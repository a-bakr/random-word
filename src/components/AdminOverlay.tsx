...
export function AdminOverlay({
  ...
  useEffect(() => { if (isAdmin && visible) load(win); }, [isAdmin, visible, win, load]);

  useEffect(() => {
    if (!isAdmin || !visible) return;
    const t = setInterval(() => load(win), 30_000);
    return () => clearInterval(t);
  }, [isAdmin, visible, win, load]);
      ...
      {visible && (
        ...
        >
          ...
          >
            ...
            {!isAdmin ? (
              <form onSubmit={handleLogin} className="flex flex-col gap-4 w-64 mx-auto mt-20">
