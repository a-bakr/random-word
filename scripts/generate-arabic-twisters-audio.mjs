#!/usr/bin/env node
// Generate WAVs for Arabic tongue twisters using Gemini Flash TTS.
// Usage:
//   GEMINI_API_KEY=... node scripts/generate-arabic-twisters-audio.mjs
//   GEMINI_API_KEY=... node scripts/generate-arabic-twisters-audio.mjs --only ar-001
//   GEMINI_API_KEY=... node scripts/generate-arabic-twisters-audio.mjs --force
...
const AR_FILE = resolve(ROOT, 'src/lib/languages/ar.ts');

const MODEL = process.env.GEMINI_TTS_MODEL || 'gemini-2.5-flash-preview-tts';
const VOICE = process.env.GEMINI_TTS_VOICE || 'Kore';
const STYLE = 'اقرأ لسان المزمار التالي ببطء ووضوح، مع النطق الصحيح لكل حرف، للمتدرب على نطق اللغة العربية:';
...
// Parse Arabic twisters from ar.ts
const src = readFileSync(AR_FILE, 'utf8');
const twisters = [];
// Match entries in ARABIC_TWISTERS array — ids start with 'ar-'
const re = /\{\s*id:\s*'(ar-\d+)',\s*text:\s*'((?:[^'\\]|\\.)*)'/g;
let m;
while ((m = re.exec(src))) {
  twisters.push({ id: m[1], text: m[2].replace(/\\n/g, ' ').replace(/\\'/g, "'") });
}
if (!twisters.length) {
  console.error('No Arabic twisters parsed from ar.ts');
  process.exit(1);
}
console.log(`Found ${twisters.length} Arabic twisters`);
...
for (const t of targets) {
  ...
  console.log(`ok   ${t.id}  ${(size / 1024).toFixed(1)} KB  "${t.text.slice(0, 60)}${t.text.length > 60 ? '…' : ''}"`);
}
